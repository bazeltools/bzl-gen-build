...
fn process_jar() {
    ...
    let relative_path = "tests/data/scala-parser-combinators_2.12-2.3.0.jar";
    ...
    let tmp_json_path = dir.path().join("integration_test_output.json");
    ...
    assert_eq!(data_block.entity_path, relative_path);
    assert!(data_block
        .defs
    ...
    assert!(!data_block
        .defs
        .iter()
        .any(|s| s.contains("anon") | s.contains("META-INF") | s.contains(".class")));
}
