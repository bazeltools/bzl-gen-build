...
object JavaSourceEntityExtractor {
  ...
  def structureOf(compUnit: CompilationUnit): DataBlock = {
    ...
    val optPack: Option[Entity] =
        compUnit.getPackageDeclaration.toScala.map { p => Entity.dotted(p.getName().toString) }
          ...
          .flatMap { t =>
            t.getFullyQualifiedName().toScala.toList.flatMap { fqn =>
              ...
              val generated = anns.asScala.toList.flatMap {
                case sc: SingleMemberAnnotationExpr if generators(sc.getName.asString) =>
                  // these drop the Def off the end and generate that type
                  assert(fqn.endsWith("Def"))
                  val nonDef = fqn.dropRight(3)
    ...
    def jtypeToEntities(jt: JType): LazyList[Entity] =
      typeCache.getOrElseUpdate(jt,
        if (jt.isArrayType()) jtypeToEntities(jt.asArrayType().getComponentType())
        else if (jt.isWildcardType()) {
            val wildcardType = jt.asWildcardType()
            wildcardType.getExtendedType().toScala.to(LazyList).flatMap(jtypeToEntities(_)) #:::
                wildcardType.getSuperType().toScala.to(LazyList).flatMap(jtypeToEntities(_))
        }
        ...
        jt.asIntersectionType().getElements()
            .asScala
            .to(LazyList)
            .flatMap(jtypeToEntities(_))
        }
    ...
    val (referencedTypes: Set[JType], refNames: Set[String]) = {
      ...
      import ast.expr.{AnnotationExpr, MethodCallExpr, NameExpr, FieldAccessExpr}
